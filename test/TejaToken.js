...
contract('TejaToken' , function(accounts){
    ...
    it('initializes the contract with the coorect values ' , function(){
      return TejaToken.deployed().then(function(instance){
        tokenInstance = instance ; 
        return tokenInstance.name();
      }).then(function(name){
        assert.equal(name, 'TejaToken' , 'correct');
        return tokenInstance.symbol();
      }).then(function(symbol){
        assert.equal(symbol, 'PKAK' , "symbol is correct")
        return tokenInstance.standard();
      }).then(function(standard){
        assert.equal(standard , 'TejaToken v1.0' , 'standard is correct' )
      })
    })
  ...
  it('transfer token ownership' , function(){
    return TejaToken.deployed().then(function(instance){
      ...
      assert(error.message.indexOf('revert') >= 0, 'error mesage must contain revert');
